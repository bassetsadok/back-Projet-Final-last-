...
//createCustomToken(uid: string, developerClaims?: undefined | object): Promise<string>

const createSendToken = async (user, status, req, res) => {
  console.log(user);
  const firebaseCustomToken = await firebaseAuth.createCustomToken(
    user._id.toString(),
    {
      role: 'user',
      email: user.email,
...
exports.createFirebaseToken = cathAsync(async (req, res, next) => {
  const firebaseCustomToken = await firebaseAuth.createCustomToken(
...
exports.superLogin = cathAsync(async (req, res, next) => {
  const { email, password } = req.body;
  ...
  // 4 ) Check if he is an admin
  if (user.role !== 'admin') {
    return next(new AppError('You have no authorization', 401));
  }
